...
#### Scenario: Approvals waiting on this buyer
- **GIVEN** orders awaiting approval in the buyer's organisation and a buyer holding an approver role
- **WHEN** the dashboard loads
- **THEN** only the approvals whose currently open tier includes one of that buyer's roles are counted as waiting on them
...
| Pending approvals count | `[MIDDLEWARE]` | Approval queue query |
| Outstanding invoices summary | `[MIDDLEWARE]` | Finance or ERP integration |
| Recent orders widget | `[MIDDLEWARE]` | Last N orders from the account |
| Pending approvals widget | `[MIDDLEWARE]` | Approval queue items |
| Quick order shortcut | `[STATIC]` | Navigation link |
