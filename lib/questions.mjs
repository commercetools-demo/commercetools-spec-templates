...
function deriveOptions(q, scopes) {
  if (!q.options_from) {
    return (q.options ?? []).map((o) => ({
      ...
      description: pickVariant(o.description_when, scopes)
        ?? (interpolate(o.description ?? "", scopes) || undefined),
    }));
  }
  ...
  if (q.options_filter?.supports_model) {
    ...
    // A paired model counts per SIDE: a seller portal and a storefront differ by ~18 specs, so a
    // note that quotes the model alone is wrong for one of them. The side is read from wherever
    // the questionnaire says it lives, and is simply absent for a single-storefront model.
    const side = q.options_filter.supports_side
      ? readPath(q.options_filter.supports_side, scopes)
      : null;
    const countKey = side ? `${model}|${side}` : String(model);
    entries = entries
      ...
      .map((e) => (e.counts ? { ...e, capability_count: e.counts[countKey] ?? 0 } : e));
  }
  ...
  for (const o of q.options_append ?? []) {
    derived.push({
      ...
      description: pickVariant(o.description_when, scopes)
        ?? (o.description ? interpolate(o.description, scopes) : undefined),
    });
  }
  return derived;
}
...
const pickVariant = (variants, scopes) => {
  for (const v of variants ?? []) {
    if (evaluate(v.when, scopes)) return interpolate(v.prompt ?? v.label ?? v.description, scopes);
  }
  return null;
};
