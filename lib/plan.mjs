 ...
 */
export function buildPlan({ cwd, framework, industry, model, sides = [], placement, scope, files, contentVersion }) {
  const receipt = readReceipt(cwd);
  // The receipt is trusted by PATH, and a paired bundle is namespaced under its side, so a
  // receipt covering the other shop simply has no entry for any path this bundle writes. That is
  // what makes holding both shops in one project safe rather than a silent overwrite.
