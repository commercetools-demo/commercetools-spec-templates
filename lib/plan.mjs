 ...
 */
export function buildPlan({ cwd, framework, industry, model, side = null, placement, scope, files, contentVersion }) {
  const receipt = readReceipt(cwd);
  // A receipt from the OTHER side of the same pair describes different specs at the same paths.
  // Treating those files as ours would quietly overwrite the other shop's specs, so the receipt
  // is only trusted when it is for this side; otherwise every path reads as `foreign` and the
  // plan blocks until the developer says --force.
  const sameBundle = !receipt || (receipt.side ?? null) === (side ?? null);
  const known = new Map(sameBundle ? (receipt?.files ?? []).map((f) => [f.path, f]) : []);
