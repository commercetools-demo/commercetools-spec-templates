...
const FUNCS = {
  ...
  // A paired model carries 8-14 gaps, and humanizing all of them renders one unreadable
  // sentence. Name a few, count the rest. `interpolate` only accepts a quoted string as a
  // second argument, so the cut-off is fixed here rather than passed in.
  brief: (v) => {
    const all = (Array.isArray(v) ? v : [v])
      .filter((x) => x !== undefined && x !== null)
      .map((x) => String(x).replace(/-/g, " "));
    return all.length <= 3 ? all.join(", ") : `${all.slice(0, 3).join(", ")}, and ${all.length - 3} more`;
  },
